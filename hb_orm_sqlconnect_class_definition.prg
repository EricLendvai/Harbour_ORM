...
class hb_orm_SQLConnect
    hidden:
        ...
        data p_TableSchemaCacheLogLastPk   init 0            // Used in case of schema caching
    ...
    exported:
        data p_TableSchema init {=>}                                     //List of Existing Tables Names. Each element is a hash with "Fields" and "Indexes" array elements ["Hash of Field Definition","Hash of Index Definitions"]. Named it with a leading "p_" to be threaded as internal.
        data p_WharfConfig init {=>}                                     //Application definition generated by DataWharf (WharfSystems in the future). This will be used to enforce OnDelete and Foreign key resolutions for example.
        ...
        method LoadSchema(par_cSource)  //Called on successful Connect(). Should be called if the Schema changed since the last Connect()
        ...
        method NormalizeTableNameInternal(par_cNamespaceAndTableName)                                      // Will clean up and adjust the table name based on Engine Type and to be used against ::p_TableSchema
