...
local l_pos
local l_FieldTags
local l_TableFullPath
//local l_CheckMarck := "✓"  //hb_utf8Chr(10003)

if !empty(par_alias) .and. (par_alias)->(used())
    l_RecordSize := (par_alias)->(RecSize())
    if l_RecordSize <= 0
        l_RecordSize := (par_alias)->(DBINFO(DBI_GETRECSIZE))
    endif

    l_TableFullPath := (par_alias)->(DBINFO(DBI_FULLPATH))
                ...
                if l_FileHandle > 0
                    ...
                    if l_DisplayStructure
                        ...
                        l_html += '<div class="TableDescription">'
                            ...
                            l_html += '<span>&nbsp;&nbsp;&nbsp;Number of Fields:</span><span>&nbsp;'+Alltrim(Str(l_NumberOfFields))+'</span>'
                            if l_RecordSize > 0    // Fails to work in SQLMix, _M_ more research needed.
                                l_html += '<span>&nbsp;&nbsp;Record size:</span><span>&nbsp;'+Alltrim(Str(l_RecordSize))+'</span>'
                            endif
                            if !empty(l_TableFullPath)  // For SQLMix table this is empty
                                l_html += '<span>&nbsp;&nbsp;Table Path :</span><span>&nbsp;'+l_TableFullPath+'</span>'
                            endif
